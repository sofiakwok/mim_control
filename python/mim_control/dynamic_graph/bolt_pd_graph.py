...
class BoltPD:
    def __init__(
        ...
        # LQR Controller
        self.pd_ctrl = mim_control_dg.BoltPD(prefix + "_pd_controller")
        self.pd_ctrl.initialize(pin_robot.model)

        # The final computed control.
        # self.joint_torques_sout = constVector(np.zeros(6))
        self.joint_torques_sout = self.pd_ctrl.joint_torque_sout

        ###
        # Export all the signals for the user of the PyEntity.
        self.des_robot_configuration_sin = self.pd_ctrl.des_robot_configuration_sin
        self.des_robot_velocity_sin = self.pd_ctrl.des_robot_velocity_sin
        print("done initializing pd_graph")
    ...
    def trace(self, robot=None):
        ...
        robot.add_trace(self.pd_ctrl.name, "torque_sout")
        robot.add_trace(self.pd_ctrl.name, "joint_torque_sout")
        robot.add_trace(self.pd_ctrl.name, "robot_configuration_sin")
        robot.add_trace(self.pd_ctrl.name, "robot_velocity_sin")
        robot.add_trace(self.pd_ctrl.name, "des_robot_configuration_sin")
        robot.add_trace(self.pd_ctrl.name, "des_robot_velocity_sin")
    ...
    def plug_all_signals(
        ...
        dg.plug(position, self.pd_ctrl.robot_configuration_sin)
        dg.plug(velocity, self.pd_ctrl.robot_velocity_sin)
    ...
    def plug_base_as_com(self, base_position, base_velocity_world):
        ...
        dg.plug(
            ...
            self.pd_ctrl.actual_com_position_sin
        )

        dg.plug(
            selec_vector(base_velocity_world, 0, 3),
            self.pd_ctrl.actual_com_velocity_sin
        )
