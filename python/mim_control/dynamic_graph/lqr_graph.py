...
class LQRController:
    def __init__(
        ...
        # LQR Controller
        self.lqr_ctrl = lqr_control_dg.LQRController(prefix + "_lqr_controller")
        print("created LQRController")
        self.lqr_ctrl.initialize(pin_robot.model)
        print("initialized pinmodel LQRController")
    ...
    def trace(self, robot=None):
        ...
        if robot is None:
            try:
                ...
                print("LQRController.trace(): No robot given, cannot trace the data.")
                return
