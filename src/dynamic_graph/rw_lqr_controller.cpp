 ...
 * Gesellschaft
 *
 * @brief Dynamic graph wrapper around the RWLQRController class.
 */
...
DYNAMICGRAPH_FACTORY_ENTITY_PLUGIN(RWLQRController, "RWLQRController");

RWLQRController::RWLQRController(const std::string& name)
    :  // Inheritance.
      ...
      define_output_signal(torque_sout_,
                           "inner",
                           one_iteration_sout_,
                           &RWLQRController::torque_callback),
      define_output_signal(joint_torque_sout_,
                           "inner",
                           one_iteration_sout_,
                           &RWLQRController::joint_torque_callback),
      // Inner signal.
      one_iteration_sout_(
          boost::bind(
              &RWLQRController::one_iteration_callback, this, _1, _2),
...
{
    rw_lqr_controller_.initialize(pinocchio_model);
}

dynamicgraph::Vector& RWLQRController::torque_callback(
    dynamicgraph::Vector& signal_data, int time)
{
    one_iteration_sout_.access(time);
    signal_data = rw_lqr_controller_.get_torques();
    return signal_data;
}

dynamicgraph::Vector& RWLQRController::joint_torque_callback(
    dynamicgraph::Vector& signal_data, int time)
{
    one_iteration_sout_.access(time);
    signal_data = rw_lqr_controller_.get_joint_torques();
    return signal_data;
}

bool& RWLQRController::one_iteration_callback(bool& signal_data, int time)
{
    ...
    rw_lqr_controller_.run(robot_configuration,
                        robot_velocity,
