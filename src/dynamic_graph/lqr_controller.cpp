...
DYNAMICGRAPH_FACTORY_ENTITY_PLUGIN(LQRController, "LQRController");

LQRController::LQRController(const std::string& name)
    :  // Inheritance.
      ...
      define_output_signal(torque_sout_,
                           "inner",
                           one_iteration_sout_,
                           &LQRController::torque_callback),
      define_output_signal(joint_torque_sout_,
                           "inner",
                           one_iteration_sout_,
                           &LQRController::joint_torque_callback),
      // Inner signal.
      one_iteration_sout_(
          boost::bind(
              &LQRController::one_iteration_callback, this, _1, _2),
...
{
    lqr_controller_.initialize(pinocchio_model);
}

dynamicgraph::Vector& LQRController::torque_callback(
    dynamicgraph::Vector& signal_data, int time)
{
    one_iteration_sout_.access(time);
    signal_data = lqr_controller_.get_torques();
    return signal_data;
}

dynamicgraph::Vector& LQRController::joint_torque_callback(
    dynamicgraph::Vector& signal_data, int time)
{
    one_iteration_sout_.access(time);
    signal_data = lqr_controller_.get_joint_torques();
    return signal_data;
}

bool& LQRController::one_iteration_callback(bool& signal_data, int time)
{
    ...
    lqr_controller_.run(robot_configuration,
                        robot_velocity,
