 ...
 * Gesellschaft
 *
 * @brief Dynamic graph wrapper around the RWImpedanceController class.
 */
...
DYNAMICGRAPH_FACTORY_ENTITY_PLUGIN(RWImpedanceController, "RWImpedanceController");

RWImpedanceController::RWImpedanceController(const std::string& name)
    :  // Inheritance.
      ...
      define_output_signal(torque_sout_,
                           "inner",
                           one_iteration_sout_,
                           &RWImpedanceController::torque_callback),
      define_output_signal(joint_torque_sout_,
                           "inner",
                           one_iteration_sout_,
                           &RWImpedanceController::joint_torque_callback),
      define_output_signal(impedance_force_,
                           "inner",
                           one_iteration_sout_,
                           &RWImpedanceController::impedance_force_callback),
      // Inner signal.
      one_iteration_sout_(
          boost::bind(
              &RWImpedanceController::one_iteration_callback, this, _1, _2),
...
void RWImpedanceController::initialize(const pinocchio::Model& pinocchio_model,
...
dynamicgraph::Vector& RWImpedanceController::torque_callback(
...
dynamicgraph::Vector& RWImpedanceController::joint_torque_callback(
    dynamicgraph::Vector& signal_data, int time)
{
    one_iteration_sout_.access(time);
...
dynamicgraph::Vector& RWImpedanceController::impedance_force_callback(
    dynamicgraph::Vector& signal_data, int time)
{
    one_iteration_sout_.access(time);
...
bool& RWImpedanceController::one_iteration_callback(bool& signal_data, int time)
{
